...
AActor *URaycastUtils::RaycastForInteractable(
    ...
    float DebugDuration)
{
  ...
  if (bHit && OutHit.GetActor())
  {
    ...
    if (HitActor->GetClass()->ImplementsInterface(InteractableInterface))
    {
      return HitActor;
    }
  }
...
TArray<AActor *> URaycastUtils::SphereTraceForInteractables(
    UObject *WorldContextObject,
    FVector Start,
    FVector End,
    float Radius,
    ...
    float DebugDuration)
{
  ...
  if (bHit)
  {
    for (const FHitResult &Hit : OutHits)
    {
      ...
      if (HitActor && HitActor->GetClass()->ImplementsInterface(InteractableInterface))
      {
...
AActor *URaycastUtils::RaycastForInteractableFromPlayer(UObject *WorldContextObject, float TraceDistance, TSubclassOf<UInterface> InteractableInterface, FHitResult &OutHit, ECollisionChannel TraceChannel, bool bDrawDebug, float DebugDuration)
{
  FVector Start, End;
  GetPlayerViewTrace(WorldContextObject, TraceDistance, Start, End);
  return RaycastForInteractable(WorldContextObject, Start, End, InteractableInterface, OutHit, TraceChannel, bDrawDebug, DebugDuration);
}

TArray<AActor *> URaycastUtils::SphereTraceForInteractablesForInteractableFromPlayer(UObject *WorldContextObject, float Radius, TSubclassOf<UInterface> InteractableInterface, TArray<FHitResult> &OutHits, ECollisionChannel TraceChannel, bool bDrawDebug, float DebugDuration)
{
  FVector Start, End;
  GetPlayerViewTrace(WorldContextObject, Radius, Start, End);
  // stationary sphere trace at Start location
  return SphereTraceForInteractables(WorldContextObject, Start, Start, Radius, InteractableInterface, OutHits, TraceChannel, bDrawDebug, DebugDuration);
}
