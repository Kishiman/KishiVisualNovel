...
struct RpyParser
{
	...
	static FRpySceneOptions GetRpySceneOptions(TArray<FString> params, int offset = 0)
	{
		...
		FString layer = params[2 + offset];
		FString positionEnum = params[3 + offset];
		FString positionVector = params[4 + offset];
		if (!with.IsEmpty())
		{
		...
		{
			options.position = GetVector(positionVector);
		}
		return options;
	}
};
...
std::string RpyParser::reg_rpy_scene_options = "(?:(?:\\s*with\\s*(" + reg_transition_enum_nc + ")\\s*(" + reg_float_nc + ")?)|(?:\\s*onlayer\\s*(" + reg_layer_nc + "))|(?:\\s*at\\s*" + reg_position + "))*";
