...
//     return;
// }
void UVisualNovelScript::ScriptTreeFromPyFines(TArray<FPyLine>& PyLines){
    UVisualNovelScriptNode* TreeStack[16];
    this->ScriptTree=NewObject<UVisualNovelScriptNode>(this);
    TreeStack[0]=(this->ScriptTree);
    for (FPyLine pyLine:PyLines)
    {
        UVisualNovelScriptNode* newNode=NewObject<UVisualNovelScriptNode>(this);
        newNode->ScriptLine=pyLine;
        TreeStack[pyLine.tabs+1]=newNode;
        TreeStack[pyLine.tabs]->AddChild(newNode);
    }
